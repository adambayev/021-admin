...
class ReactMultiSelect extends React.Component {
  ...
  render() {
    ...
    let selectedList = (
      <label
        className="selected-options-badges-list"
        onClick={() => {
      ...
      >
        {selected.map(obj => {
          return (
            <span
              style={this.props.selectedOptionsStyles || selectedOptionsStyles}
              key={obj.id}
              onClick={this.selectedOptionsClick.bind(this, obj.id)}
              className="selected-options-badges"
            >
              {obj.name}
            </span>
          );
            ...
            return (
              <li key={el.id} value={el.value}>
                <div
                  ...
                  style={
                    el.value
                      ? this.props.optionsListStyles || optionsListStyles
                      : {}
                  }
                  onClick={this.optionsOnchange.bind(this, i, !el.value)}
                >
                  {el.name}
                </div>
              </li>
